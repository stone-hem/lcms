...
const openEdit = (item: any) => { isEdit.value = true; form.id = item.id; form.name = item.name; form.description = item.description || ''; showModal.value = true }

const submit = () => {
  isEdit.value
    ? form.put(route('misc.update_party_type', form.id), { onSuccess: () => showModal.value = false })
    : form.post(route('misc.store_party_type'), { onSuccess: () => showModal.value = false })
}

const deactivate = (id: number) => confirm('Deactivate?') && router.post(route('misc.deactivate_party_type', id))
const activate = (id: number) => router.post(route('misc.activate_party_type', id))
const destroy = (id: number) => confirm('Delete permanently?') && router.post(route('misc.delete_party_type', id))
</script>
...
<template>
  <Card>
    <CardHeader class="flex flex-row items-center justify-between">
      ...
      <Button @click="openAdd">Add Party Type</Button>
    </CardHeader>
    <CardContent>
  ...
  <Dialog :open="showModal" @update:open="showModal = false">
    <DialogContent>
      <DialogHeader>
        <DialogTitle>{{ isEdit ? 'Edit' : 'Add' }} Party Type</DialogTitle>
      </DialogHeader>
      <form @submit.prevent="submit" class="space-y-4">
        <div>
          <Label>Name <span class="text-red-600">*</span></Label>
          <Input v-model="form.name" required placeholder="e.g. Plaintiff, Defendant" />
        </div>
        <div>
