...
const form = useForm({ id: null, type: '', description: '' })

const openAdd = () => { isEdit.value = false; form.reset(); showModal.value = true }
const openEdit = (item: any) => { isEdit.value = true; form.id = item.id; form.type = item.type; form.description = item.description || ''; showModal.value = true }

const submit = () => {
  isEdit.value
    ? form.put(route('misc.update_expense_type', form.id), { onSuccess: () => showModal.value = false })
    : form.post(route('misc.store_expense_type'), { onSuccess: () => showModal.value = false })
}

const deactivate = (id: number) => confirm('Deactivate?') && router.post(route('misc.deactivate_expense_type', id))
const activate = (id: number) => router.post(route('misc.activate_expense_type', id))
const destroy = (id: number) => confirm('Delete permanently?') && router.post(route('misc.delete_expense_type', id))
</script>
...
<template>
  <Card>
    <CardHeader class="flex flex-row items-center justify-between">
      ...
      <Button @click="openAdd">Add Expense Type</Button>
    </CardHeader>
    <CardContent>
  ...
  <Dialog :open="showModal" @update:open="showModal = false">
    <DialogContent>
      <DialogHeader>
        <DialogTitle>{{ isEdit ? 'Edit' : 'Add' }} Expense Type</DialogTitle>
      </DialogHeader>
      <form @submit.prevent="submit" class="space-y-4">
        <div>
          <Label>Type <span class="text-red-600">*</span></Label>
          <Input v-model="form.type" required placeholder="e.g. Filing Fees" />
        </div>
        <div>
