...
import { Calendar, Filter, RefreshCw, Menu, CheckSquare, Activity, CalendarDays } from 'lucide-vue-next'
...
const refresh = (startDate: string | null = null, endDate: string | null = null) => {
  ...
  // Add type filters based on current typeFilter value
  if (typeFilter.value === -1) {
    // Show all types
    params.display_tasks = 1
    params.display_events = 1
    params.display_activities = 1
  } else if (typeFilter.value === 1) {
    // Show only tasks
    params.display_tasks = 1
    params.display_events = 0
    params.display_activities = 0
  } else if (typeFilter.value === 2) {
    // Show only activities
    params.display_tasks = 0
    params.display_events = 0
    params.display_activities = 1
  ...
  const url = '/calendar/master?' + new URLSearchParams(params);
...
<template>
  <AppLayout>
    <Head :title="'Master Calendar'" />

    <div class="flex md:space-x-5 app_height overflow-hidden relative rtl:space-x-reverse md:p-8 p-4">
      <!-- Sidebar -->
      <div
        class="transition-all duration-300 z-10 bg-white dark:bg-black h-full"
            ...
            <div class="space-y-3">
              ...
              <button
                ...
                :class="typeFilter === -1
                  ...
                  : 'hover:bg-slate-100 dark:hover:bg-slate-700 text-gray-700 dark:text-gray-200'"
              >
      ...
      <div class="flex-1">
        <Card bodyClass="p-0 h-full">
          <div class="p-4 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center">
            <h3 class="text-lg font-semibold">Master Calendar</h3>
