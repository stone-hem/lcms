...
class SettingsController extends Controller
{
    ...
    public function external_firms(Request $request)
    {
        ...
        $items = ExternalFirm::with("user")->withTrashed();
        if ($search_query) {
            $title_where_array = QueryHelper::get_where_clause_with_match_mode(
                "contains",
                $search_query,
                "external_firms.firm_name"
            );
    ...
    // Store – now returns Inertia redirect + flash
    public function store_external_firm(Request $request)
    {
        $request->validate([
            'firm_name'           => 'required|min:2|unique:external_firms',
            'bank_name'           => 'required|min:2',
            'bank_branch'         => 'required|min:2',
            'bank_account_number' => 'required|min:2|unique:external_firms',
            'postal_address'      => 'required|min:2|unique:external_firms',
            'kra_pin'             => 'required|min:4|unique:external_firms',
        ...
        return redirect()->back()
            ->with('success', 'External counsel created successfully');
    }
    public function update_external_firm(Request $request, $id)
    {
        $firm = ExternalFirm::withTrashed()->findOrFail($id);
    
        $request->validate([
            'firm_name'           => 'required|min:2|unique:external_firms,firm_name,' . $id,
            'bank_name'           => 'required|min:2',
            'bank_branch'         => 'required|min:2',
            'bank_account_number' => 'required|min:2|unique:external_firms,bank_account_number,' . $id,
            'postal_address'      => 'required|min:2|unique:external_firms,postal_address,' . $id,
            'kra_pin'             => 'required|min:4|unique:external_firms,kra_pin,' . $id,
            'email'               => 'nullable|email|unique:users,email,' . ($firm->user_id ?? ''),
            'new_password'        => 'nullable|min:6',
        ]);
    
        $firm->update($request->only([
            'firm_name', 'bank_name', 'bank_branch',
            'bank_account_number', 'postal_address', 'kra_pin'
        ]));
    
        if ($firm->user_id) {
            $user = \App\Models\User::withTrashed()->find($firm->user_id);
            $user->email = $request->email ?? $user->email;
            if ($request->filled('new_password')) {
                $user->password = \Hash::make($request->new_password);
            }
            $user->deleted_at = $request->can_login ? null : now();
            $user->save();
        } elseif ($request->can_login && $request->filled('email') && $request->filled('password')) {
            $user = \App\Models\User::create([
                'first_name'        => $request->firm_name,
                'email'             => $request->email,
                'password'          => \Hash::make($request->password),
        ...
        return redirect()->route('counsel.external')
            ->with('success', 'External counsel updated successfully');
    }
    
    public function activate_external_firm($id)
    {
        $item = ExternalFirm::withTrashed()->find($id);
        if ($item) {
            $item->restore();
        }
        if ($item->user_id) {
        ...
        return response()->json([
            "error" => false,
            "message" => "External firm activated successfully",
        ]);
    }
    public function deactivate_external_firm($id)
    {
        $item = ExternalFirm::withTrashed()->find($id);
        ...
        return response()->json([
            "error" => false,
            "message" => "External firm deactivated successfully",
        ]);
    }
    public function delete_external_firm($id)
    {
        $item = ExternalFirm::withTrashed()->find($id);
        ...
        return response()->json([
            "error" => false,
            "message" => "External deleted successfully",
        ]);
    }
